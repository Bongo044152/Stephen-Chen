...
class ProfessorParser:
    ...
    def parse_html(html):
        ...
        professor_items = soup.select('.i-member-item.col-md-6')
        
        professors = []
        for item in professor_items:
            li_elements = item.select('li')
    ...
    @staticmethod
    def _extract_professor_info(li_elements, html_content) -> dict:
        """
        從 HTML 元素中提取教授資訊
        
        Args:
            li_elements (list): <li> 元素列表
            html_content (str): 原始 HTML 內容
                ...
                if field in li.text:
                    if field == '研究領域':
                        ...
                        try:
                            ...
                            my_logger.warning(f"解析 'Office hour' 時出錯: {e}")
                            pass # 這個錯誤並不重要，可忽視
                    elif field == '經歷':
                        try:
                            span = li.select_one('span:nth-of-type(2)') # 選取第二個 span
                            if span:
                                temp = str(span)
